...
/********************** Prototypes *********************/
static void menu_item_activate(guint);

static GtkTreeModel *build_file_list(const gchar *, const gchar *);

static void directory_check(GtkEntry *, GtkEntryCompletion *);

static GtkWidget *create_dialog(GtkWidget **, GtkTreeModel *);
...
void goto_file_init(void)
{
	...
	keybindings_set_item(plugin_key_group, KEY_ID_GOTO_FILE,
						 (GeanyKeyCallback)(&menu_item_activate),
						 GDK_g, GDK_MOD1_MASK | GDK_SHIFT_MASK,
						 "goto_file", _("Go to File"),	/* used in the Preferences dialog */
						 menu_item);
}
 ...
 * @return	GtkTreeModel *
 */
static GtkTreeModel *build_file_list(const gchar *dirname, const gchar *prefix)
{
	GtkListStore *ret_list = gtk_list_store_new (1, G_TYPE_STRING);
	
	/* used to free later the sub-elements */
	guint files_n;
	GSList *files_list = utils_get_file_list(dirname, &files_n, NULL);
	
	for (GSList *file_iterator = files_list; file_iterator;
		file_iterator = file_iterator->next) 
	{
		gchar *file = file_iterator->data;
		gchar *pathfile = g_build_filename(dirname, file, NULL);   
		
		/* Append the element to model list */
		GtkTreeIter iter;
		gtk_list_store_append(ret_list, &iter);
		gtk_list_store_set(ret_list, &iter, 0, g_strconcat(prefix, file, NULL), -1);
		g_free(pathfile);
	}
 ...
 * @return	void
 */
static void directory_check(GtkEntry *entry, GtkEntryCompletion *completion)
{
	...
	gchar *new_dir = g_strndup(text, dir_sep + 1);
	...
	SETPTR(curr_dir, new_dir);
	
	/* Save the completion_mode for future restore. */
	if (!old_model) old_model = gtk_entry_completion_get_model(completion);
	
	log_debug("New completion list!");
	
	gchar *new_dir_path = g_path_is_absolute(new_dir) ? new_dir
													  : g_build_filename(directory_ref,
																		 new_dir, NULL);
	/* Build the new file list for completion */
	GtkTreeModel *completion_list = build_file_list(new_dir_path, new_dir);
...
/**
 * @brief	Create the dialog, return the entry object to get the response from user 
 * @param	GtkWidget **dialog				entry object
 * @param	GtkTreeModel *completion_model	completion object
 * 
 * @return	GtkWidget *entry
 */
static GtkWidget *create_dialog(GtkWidget **dialog, GtkTreeModel *completion_model)
{
	*dialog = gtk_dialog_new_with_buttons(_("Go to File..."),
	...
	GtkWidget *label = gtk_label_new(_("Enter the file you want to open:"));
...
static void menu_item_activate(guint key_id)
{
	...
	GtkTreeModel *completion_list = build_file_list(directory_ref, "");
	
	/* Create the user dialog and get response */
	GtkWidget *dialog;
	GtkWidget *dialog_entry = create_dialog(&dialog, completion_list);
	gint response = gtk_dialog_run(GTK_DIALOG(dialog));
	
	/* Filename */
	const gchar *chosen_file = gtk_entry_get_text(GTK_ENTRY(dialog_entry));
	/* Path + Filename */
	gchar *chosen_path = g_build_filename(directory_ref, chosen_file, NULL);
	...
	if (response == GTK_RESPONSE_ACCEPT)
	{
		...
		if (!g_file_test(chosen_path, G_FILE_TEST_EXISTS))
		{
			...
			GtkWidget *dialog_new = gtk_message_dialog_new(
											GTK_WINDOW(geany->main_widgets->window),
