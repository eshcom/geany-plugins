...
static Pastebin *pastebin_new(const gchar *filepath, GError **error)
{
	gboolean result = FALSE;
	GKeyFile *kf = load_config_from_file(filepath, &result);
...
static const Pastebin *find_pastebin_by_name(const gchar *name)
{
	...
	for (GSList *node = pastebins; node; node = node->next)
	{
...
{
	gchar *paths[] = {
		get_config_filepath(PLUGIN, "pastebins"),
		get_data_filepath(PLUGIN, "pastebins")
	};
...
static void load_settings(void)
{
	...
	if (g_key_file_has_key(config, CONFIG_SECTION, "pastebin", NULL) ||
		!g_key_file_has_key(config, CONFIG_SECTION, "website", NULL))
	{
		pastebin_selected = utils_get_setting_string(config, CONFIG_SECTION,
													 "pastebin", "pastebin.geany.org");
	}
	else
	{	/* compatibility for old setting "website" */
		switch (utils_get_setting_integer(config, CONFIG_SECTION, "website", 2))
		{
	...
	check_button_is_checked = utils_get_setting_boolean(config, CONFIG_SECTION,
														"open_browser", FALSE);
	author_name = utils_get_setting_string(config, CONFIG_SECTION,
...
static void save_settings(void)
{
	...
	g_key_file_set_string(config, CONFIG_SECTION, "pastebin", pastebin_selected);
	g_key_file_set_boolean(config, CONFIG_SECTION, "open_browser", check_button_is_checked);
	g_key_file_set_string(config, CONFIG_SECTION, "author_name", author_name);
								  ...
								  gpointer *user_data)
{
	...
	{
		SETPTR(pastebin_selected, gtk_combo_box_text_get_active_text(
										GTK_COMBO_BOX_TEXT(widgets.combo)));
		check_button_is_checked = gtk_toggle_button_get_active(
