...
static void load_config(void)
{
	...
	if (result)
	{
		vi_set_enabled(utils_get_setting_boolean(config, CONF_GROUP, CONF_ENABLE_VIM,
												 TRUE));
		vi_set_insert_for_dummies(utils_get_setting_boolean(config, CONF_GROUP,
															CONF_INSERT_FOR_DUMMIES,
															FALSE));
		start_in_insert = utils_get_setting_boolean(config, CONF_GROUP,
													CONF_START_IN_INSERT, FALSE);
	}
...
static void save_config(void)
{
	...
	g_key_file_set_boolean(config, CONF_GROUP, CONF_ENABLE_VIM, vi_get_enabled());
	g_key_file_set_boolean(config, CONF_GROUP, CONF_INSERT_FOR_DUMMIES,
						   vi_get_insert_for_dummies());
	g_key_file_set_boolean(config, CONF_GROUP, CONF_START_IN_INSERT, start_in_insert);
...
{
	gboolean enabled = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(
												menu_items.enable_vim_item));
	vi_set_enabled(enabled);
	vi_set_mode(start_in_insert ? VI_MODE_INSERT : VI_MODE_COMMAND);
	
	if (!enabled) ui_set_statusbar(FALSE, "Vim Mode Disabled");
	save_config();
}
...
{
	gboolean enabled = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(
										menu_items.insert_for_dummies_item));
	vi_set_insert_for_dummies(enabled);
	ui_set_statusbar(FALSE, _("Insert Mode for Dummies: %s"),
					 enabled ? _("ON") : _("OFF"));
	save_config();
}


static gboolean on_insert_for_dummies_kb(GeanyKeyBinding *kb, guint key_id,
										 gpointer data)
{
...
{
	gboolean enabled = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(
										menu_items.start_in_insert_item));
...
{
	GeanyDocument *doc = document_get_current();
	if (doc) return document_save_file(doc, force);
	return TRUE;
}
...
void plugin_init(GeanyData *data)
{
	...
	cb.on_quit = on_quit;
	vi_init(geany->main_widgets->window, &cb);
	vi_set_mode(start_in_insert ? VI_MODE_INSERT : VI_MODE_COMMAND);
	
	if (doc) vi_set_active_sci(doc->editor->sci);
}
