...
#define AO_COPY_FILE_NAME_GET_PRIVATE(obj)						\
	(G_TYPE_INSTANCE_GET_PRIVATE((obj), AO_COPY_FILE_NAME_TYPE,	\
								 AoCopyFileNamePrivate))
...
void ao_copy_file_name_copy(AoCopyFileName *self)
{
	...
	if (doc != NULL)
	{
		...
		ui_set_statusbar(TRUE, _("File name \"%s\" copied to clipboard."),
						 file_name);
		g_free(file_name);
	}
}

static void copy_file_name_activated_cb(GtkMenuItem *item,
										AoCopyFileName *self)
{
