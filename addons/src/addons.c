...
{
	GtkWidget *check_openuri, *check_systray, *check_bookmarklist,
			  *check_blanklines, *check_xmltagging, *check_colortip,
			  *check_double_click_color_chooser, *check_enclose_words_auto;
	GtkWidget *check_enclose_words, *enclose_words_config_button;
	GtkWidget *vbox_tasks, *check_tasks, *check_tasks_scan_mode, *entry_tasks_tokens;
	GtkWidget *vbox_markword, *check_markword, *check_markword_single_click_deselect;
	GtkWidget *check_doclist, *vbox_doclist, *radio_doclist_name,
			  *radio_doclist_tab_order, *radio_doclist_tab_order_reversed;
	
	DoubleWidget double_widget;
	GtkWidget *vbox = gtk_vbox_new(FALSE, 6);
	
	//----------------------------------------------------------------
	double_widget = add_checked_vbox(vbox,
		_("Show toolbar item to show a list of currently open documents"),
		ao_info->enable_doclist, NULL);
	
	check_doclist = double_widget.widget1;
	vbox_doclist = double_widget.widget2;
	
	g_signal_connect(check_doclist, "toggled",
					 G_CALLBACK(ao_configure_doclist_toggled_cb), dialog);
	
	radio_doclist_name = add_init_radiobox(vbox_doclist,
		_("Sort documents by _name"),
		_("Sort the documents in the list by their filename"), TRUE);
	
	radio_doclist_tab_order = add_next_radiobox(vbox_doclist,
		radio_doclist_name, _("Sort documents by _occurrence"),
		_("Sort the documents in the order of the document tabs"), TRUE);
	
	radio_doclist_tab_order_reversed = add_next_radiobox(vbox_doclist,
		radio_doclist_name, _("Sort documents by _occurrence (reversed)"),
		_("Sort the documents in the order of the document tabs (reversed)"), TRUE);
	...
	//----------------------------------------------------------------
	check_openuri = add_checkbox(vbox,
		_("Show an 'Open URI' item in the editor menu"),
		ao_info->enable_openuri, NULL, FALSE);
	
	//----------------------------------------------------------------
	double_widget = add_checked_vbox(vbox,
		_("Show available Tasks in the Messages Window"),
		ao_info->enable_tasks, NULL);
	
	check_tasks = double_widget.widget1;
	vbox_tasks = double_widget.widget2;
	...
	//----------------------------------------------------------------
	check_systray = add_checkbox(vbox,
		_("Show status icon in the Notification Area"),
		ao_info->enable_systray, NULL, FALSE);
	
	//----------------------------------------------------------------
	check_bookmarklist = add_checkbox(vbox,
		_("Show defined bookmarks (marked lines) in the sidebar"),
		ao_info->enable_bookmarklist, NULL, FALSE);
	
	//----------------------------------------------------------------
	double_widget = add_checked_vbox(vbox,
		_("Mark all occurrences of a word when double-clicking it"),
		ao_info->enable_markword, NULL);
	
	check_markword = double_widget.widget1;
	vbox_markword = double_widget.widget2;
	...
	//----------------------------------------------------------------
	check_blanklines = add_checkbox(vbox, _("Strip trailing blank lines"),
									ao_info->strip_trailing_blank_lines,
									NULL, FALSE);
	
	//----------------------------------------------------------------
	check_xmltagging = add_checkbox(vbox, _("XML tagging for selection"),
									ao_info->enable_xmltagging, NULL, FALSE);
	
	//----------------------------------------------------------------
	double_widget = add_checkbutton(vbox,
		_("Enclose selection on configurable keybindings:"),
		_("Configure enclose pairs"), -1, ao_info->enable_enclose_words,
		NULL, FALSE);
	
	check_enclose_words = double_widget.widget1;
	enclose_words_config_button = double_widget.widget2;
	
	g_signal_connect(enclose_words_config_button, "clicked",
					 G_CALLBACK(ao_enclose_words_config), dialog);
	
	//----------------------------------------------------------------
	check_enclose_words_auto = add_checkbox(vbox,
		_("Enclose selection automatically (without having to press a keybinding)"),
		ao_info->enable_enclose_words_auto, NULL, FALSE);
	
	//----------------------------------------------------------------
	check_colortip = add_checkbox(vbox,
		_("Show a calltip when hovering over a color value"),
		ao_info->enable_colortip, NULL, FALSE);
	
	//----------------------------------------------------------------
	check_double_click_color_chooser = add_checkbox(vbox,
		_("Open Color Chooser when double-clicking a color value"),
		ao_info->enable_double_click_color_chooser, NULL, FALSE);
	
	//----------------------------------------------------------------
	g_object_set_data(G_OBJECT(dialog), "vbox_doclist", vbox_doclist);
	...
	g_object_set_data(G_OBJECT(dialog), "vbox_tasks", vbox_tasks);
	...
	g_object_set_data(G_OBJECT(dialog), "vbox_markword", vbox_markword);
