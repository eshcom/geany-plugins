...
static void ao_document_activate_cb(GObject *obj, GeanyDocument *doc,
									gpointer data)
{
	g_return_if_fail(doc != NULL && doc->is_valid);
...
static void ao_document_new_cb(GObject *obj, GeanyDocument *doc,
							   gpointer data)
{
	g_return_if_fail(doc != NULL && doc->is_valid);
...
static void ao_document_open_cb(GObject *obj, GeanyDocument *doc,
								gpointer data)
{
	g_return_if_fail(doc != NULL && doc->is_valid);
...
static void ao_document_close_cb(GObject *obj, GeanyDocument *doc,
								 gpointer data)
{
	g_return_if_fail(doc != NULL && doc->is_valid);
...
static void ao_document_save_cb(GObject *obj, GeanyDocument *doc,
								gpointer data)
{
	g_return_if_fail(doc != NULL && doc->is_valid);
...
static void ao_document_before_save_cb(GObject *obj, GeanyDocument *doc,
									   gpointer data)
{
	g_return_if_fail(doc != NULL && doc->is_valid);
...
static void ao_document_reload_cb(GObject *obj, GeanyDocument *doc,
								  gpointer data)
{
	g_return_if_fail(doc != NULL && doc->is_valid);
...
GtkWidget *ao_image_menu_item_new(const gchar *stock_id, const gchar *label)
{
	...
	GtkWidget *image = gtk_image_new_from_icon_name(stock_id,
													GTK_ICON_SIZE_MENU);
										  ...
										  gpointer data)
{
	...
	gtk_widget_set_sensitive(g_object_get_data(G_OBJECT(data),
						"check_tasks_scan_mode"), sens);
	gtk_widget_set_sensitive(g_object_get_data(G_OBJECT(data),
						"entry_tasks_tokens"), sens);
}
											 ...
											 gpointer data)
{
	...
	gtk_widget_set_sensitive(g_object_get_data(G_OBJECT(data),
						"check_markword_single_click_deselect"), sens);
}
									 ...
									 gpointer user_data)
{
	if (response == GTK_RESPONSE_OK || response == GTK_RESPONSE_APPLY)
	{
		...
		g_key_file_set_boolean(config, "addons", "show_toolbar_doclist_item",
			ao_info->enable_doclist);
		g_key_file_set_integer(config, "addons", "doclist_sort_mode",
			ao_info->doclist_sort_mode);
		g_key_file_set_boolean(config, "addons", "enable_openuri",
			ao_info->enable_openuri);
		g_key_file_set_boolean(config, "addons", "enable_tasks",
			ao_info->enable_tasks);
		g_key_file_set_string(config, "addons", "tasks_token_list",
			ao_info->tasks_token_list);
		g_key_file_set_boolean(config, "addons", "tasks_scan_all_documents",
			ao_info->tasks_scan_all_documents);
		g_key_file_set_boolean(config, "addons", "enable_systray",
			ao_info->enable_systray);
		g_key_file_set_boolean(config, "addons", "enable_bookmarklist",
			ao_info->enable_bookmarklist);
		g_key_file_set_boolean(config, "addons", "enable_markword",
		...
		g_key_file_set_boolean(config, "addons", "enable_enclose_words_auto",
			ao_info->enable_enclose_words_auto);
		g_key_file_set_boolean(config, "addons", "enable_colortip",
...
/* Initialization */
static gboolean plugin_addons_init(GeanyPlugin *plugin,
								   G_GNUC_UNUSED gpointer pdata)
{
	...
	ao_info->doclist = ao_doc_list_new(ao_info->enable_doclist,
									   ao_info->doclist_sort_mode);
	...
	/* setup keybindings */
	GeanyKeyGroup *key_group = plugin_set_key_group(geany_plugin, "addons",
	...
	GtkWidget *ao_copy_file_name_menu_item = ao_copy_file_name_get_menu_item(
														ao_info->copyfilename);
	...
	GtkWidget *ao_copy_file_path_menu_item = ao_copy_file_path_get_menu_item(
														ao_info->copyfilepath);
...
/* Configuration */
static GtkWidget *plugin_addons_configure(G_GNUC_UNUSED GeanyPlugin *plugin,
										  GtkDialog *dialog,
										  G_GNUC_UNUSED gpointer pdata)
{
	GtkWidget *vbox, *check_openuri, *check_tasks, *check_systray;
	GtkWidget *check_doclist, *vbox_doclist, *frame_doclist;
	GtkWidget *radio_doclist_name, *radio_doclist_tab_order,
	...
	GtkWidget *check_tasks_scan_mode, *entry_tasks_tokens,
			  *label_tasks_tokens, *tokens_hbox;
	GtkWidget *check_blanklines, *check_xmltagging;
	GtkWidget *check_enclose_words, *check_enclose_words_auto,
			  *enclose_words_config_button, *enclose_words_hbox;
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(check_doclist),
								 ao_info->enable_doclist);
	g_signal_connect(check_doclist, "toggled",
					 G_CALLBACK(ao_configure_doclist_toggled_cb), dialog);
	
	radio_doclist_name = gtk_radio_button_new_with_mnemonic(NULL,
											_("Sort documents by _name"));
	...
	radio_doclist_tab_order_reversed = gtk_radio_button_new_with_mnemonic_from_widget(
		GTK_RADIO_BUTTON(radio_doclist_name),
		_("Sort documents by _occurrence (reversed)"));
	...
	switch (ao_info->doclist_sort_mode)
	{
		case DOCLIST_SORT_BY_NAME:
			gtk_toggle_button_set_active(
					GTK_TOGGLE_BUTTON(radio_doclist_name), TRUE);
			break;
		case DOCLIST_SORT_BY_TAB_ORDER_REVERSE:
			gtk_toggle_button_set_active(
					GTK_TOGGLE_BUTTON(radio_doclist_tab_order_reversed), TRUE);
			break;
		case DOCLIST_SORT_BY_TAB_ORDER:
		default:
			gtk_toggle_button_set_active(
					GTK_TOGGLE_BUTTON(radio_doclist_tab_order), TRUE);
			break;
	}
	
	vbox_doclist = gtk_vbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox_doclist), radio_doclist_name,
					   FALSE, FALSE, 3);
	gtk_box_pack_start(GTK_BOX(vbox_doclist), radio_doclist_tab_order,
					   TRUE, TRUE, 3);
	gtk_box_pack_start(GTK_BOX(vbox_doclist), radio_doclist_tab_order_reversed,
					   TRUE, TRUE, 3);
	...
	gtk_widget_set_tooltip_text(check_tasks_scan_mode,
		_("Whether to show the tasks of all open documents in the list "
		  "or only those of the current document."));
	
	entry_tasks_tokens = gtk_entry_new();
	if (!EMPTY(ao_info->tasks_token_list))
		gtk_entry_set_text(GTK_ENTRY(entry_tasks_tokens),
						   ao_info->tasks_token_list);
	...
	gtk_label_set_mnemonic_widget(GTK_LABEL(label_tasks_tokens),
								  entry_tasks_tokens);
	
	tokens_hbox = gtk_hbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(tokens_hbox), label_tasks_tokens,
					   FALSE, FALSE, 3);
	gtk_box_pack_start(GTK_BOX(tokens_hbox), entry_tasks_tokens,
					   TRUE, TRUE, 3);
	
	vbox_tasks = gtk_vbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox_tasks), check_tasks_scan_mode,
					   FALSE, FALSE, 3);
	...
	gtk_toggle_button_set_active(
					GTK_TOGGLE_BUTTON(check_markword_single_click_deselect),
					ao_info->enable_markword_single_click_deselect);
	
	frame_markword = gtk_frame_new(NULL);
	gtk_frame_set_label_widget(GTK_FRAME(frame_markword), check_markword);
	gtk_container_add(GTK_CONTAINER(frame_markword),
					  check_markword_single_click_deselect);
	...
	enclose_words_config_button = gtk_button_new_with_label(
										_("Configure enclose pairs"));
	g_signal_connect(enclose_words_config_button, "clicked",
					 G_CALLBACK(ao_enclose_words_config), dialog);
	enclose_words_hbox = gtk_hbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(enclose_words_hbox), check_enclose_words,
					   FALSE, FALSE, 3);
	gtk_box_pack_start(GTK_BOX(enclose_words_hbox), enclose_words_config_button,
					   TRUE, TRUE, 3);
	...
	gtk_toggle_button_set_active(
					GTK_TOGGLE_BUTTON(check_double_click_color_chooser),
					ao_info->enable_double_click_color_chooser);
	gtk_box_pack_start(GTK_BOX(vbox), check_double_click_color_chooser,
					   FALSE, FALSE, 3);
	
	g_object_set_data(G_OBJECT(dialog), "check_doclist", check_doclist);
	g_object_set_data(G_OBJECT(dialog), "radio_doclist_name",
					  radio_doclist_name);
	g_object_set_data(G_OBJECT(dialog), "radio_doclist_tab_order",
					  radio_doclist_tab_order);
	...
	g_object_set_data(G_OBJECT(dialog), "entry_tasks_tokens",
					  entry_tasks_tokens);
	g_object_set_data(G_OBJECT(dialog), "check_tasks_scan_mode",
					  check_tasks_scan_mode);
	g_object_set_data(G_OBJECT(dialog), "check_systray", check_systray);
	g_object_set_data(G_OBJECT(dialog), "check_bookmarklist",
					  check_bookmarklist);
	...
	g_object_set_data(G_OBJECT(dialog), "check_enclose_words",
					  check_enclose_words);
	g_object_set_data(G_OBJECT(dialog), "check_enclose_words_auto",
					  check_enclose_words_auto);
	g_object_set_data(G_OBJECT(dialog), "enclose_words_config_button",
					  enclose_words_config_button);
	...
	g_signal_connect(dialog, "response",
					 G_CALLBACK(ao_configure_response_cb), NULL);
...
/* Cleanup */
static void plugin_addons_cleanup(G_GNUC_UNUSED GeanyPlugin *plugin,
								  G_GNUC_UNUSED gpointer pdata)
{
...
/* Show help */
static void plugin_addons_help(G_GNUC_UNUSED GeanyPlugin *plugin,
							   G_GNUC_UNUSED gpointer pdata)
{
...
void geany_load_module(GeanyPlugin *plugin)
{
	...
	plugin->info->author = "Enrico Tröger, Bert Vermeulen, "
						   "Eugene Arshinov, Frank Lanitz";
	/* Set functions */
