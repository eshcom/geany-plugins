...
void devhelp_plugin_load_settings(DevhelpPlugin *self, const gchar *filename)
{
	...
	if (g_key_file_has_group(kf, "webview"))
	{
		...
		{
			gchar *custom_homepage = g_key_file_get_string(kf, "webview",
														   "custom_homepage", NULL);
			if (custom_homepage)
			{
				if (strlen(custom_homepage) == 0)
				{
	...
	if (g_key_file_has_group(kf, "doc_providers"))
	{
		if (g_key_file_has_key(kf, "doc_providers", "devhelp", NULL))
		{
			...
			error ? g_error_free(error) : devhelp_plugin_set_use_devhelp(self, value);
		}
		...
		if (g_key_file_has_key(kf, "doc_providers", "man_pages", NULL))
		{
			...
			error ? g_error_free(error) : devhelp_plugin_set_use_man(self, value);
		}
		...
		if (g_key_file_has_key(kf, "doc_providers", "codesearch", NULL))
		{
			...
			error ? g_error_free(error) : devhelp_plugin_set_use_codesearch(self, value);
		}
	} /* doc_providers group */
	...
	if (g_key_file_has_group(kf, "devhelp"))
	{
		if (g_key_file_has_key(kf, "devhelp", "show_devhelp_sidebar", NULL))
		{
			...
			error ? g_error_free(error) : devhelp_plugin_set_devhelp_sidebar_visible(self, value);
		}
		...
		if (g_key_file_has_key(kf, "devhelp", "set_sidebar_tabs_bottom", NULL))
		{
			...
			error ? g_error_free(error) : devhelp_plugin_set_sidebar_tabs_bottom(self, value);
		}
	...
	if (g_key_file_has_group(kf, "man_pages"))
	{
		...
		{
			error = NULL;
			gchar *man_path = g_key_file_get_string(kf, "man_pages", "prog_path", &error);
			if (error)
				g_error_free(error);
		...
		{
			error = NULL;
			gchar *pager = g_key_file_get_string(kf, "man_pages", "pager_prog", &error);
			if (error)
				g_error_free(error);
		...
		{
			error = NULL;
			gchar *sect = g_key_file_get_string(kf, "man_pages", "section_order", &error);
			if (error)
				g_error_free(error);
	...
	{
		error = NULL;
		gint pos = g_key_file_get_integer(kf, "misc", "main_notebook_tab_pos", &error);
		if (error)
			g_error_free(error);
		else
		{
...
void devhelp_plugin_store_settings(DevhelpPlugin *self, const gchar *filename)
{
	...
	GKeyFile *kf = self->priv->kf;
	
	g_key_file_set_integer(kf, "webview", "location", (gint)self->priv->location);
	g_key_file_set_boolean(kf, "webview", "focus_webview_on_search",
						   self->priv->focus_webview_on_search);
	g_key_file_set_string(kf, "webview", "custom_homepage",
						  self->priv->custom_homepage ? self->priv->custom_homepage : "");
	...
	g_key_file_set_boolean(kf, "devhelp", "set_sidebar_tabs_bottom",
						   devhelp_plugin_get_sidebar_tabs_bottom(self));
	g_key_file_set_boolean(kf, "devhelp", "focus_sidebar_on_search",
						   self->priv->focus_sidebar_on_search);
	...
	g_key_file_set_string(kf, "codesearch", "uri_params",
						  self->priv->codesearch_params ? self->priv->codesearch_params : "");
	g_key_file_set_boolean(kf, "codesearch", "use_lang_for_search",
						   self->priv->codesearch_use_lang);
	...
	write_config_to_file(kf, filename, MSGBOX);
}
