...
static void search_mark_in_range(GeanyEditor *editor, gint flags,
...
static gboolean automark(gpointer user_data)
{
	...
	if (sci_has_selection(sci)) return FALSE;
	
	gchar *text = get_current_word(editor->sci);
	...
	search_mark_in_range(editor, SCFIND_MATCHCASE | SCFIND_WHOLEWORD, &ttf);
	g_free(text);
...
static gboolean plugin_automark_init(GeanyPlugin *plugin,
...
static void plugin_automark_cleanup(G_GNUC_UNUSED GeanyPlugin *plugin,
									G_GNUC_UNUSED gpointer pdata)
{
	if (source_id) g_source_remove(source_id);
}
...
static void plugin_automark_help(G_GNUC_UNUSED GeanyPlugin *plugin,
								 G_GNUC_UNUSED gpointer pdata)
{
