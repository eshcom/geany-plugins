...
static void scp_free_element(ScpTreeStore *store, AElem *elem)
{
	...
	for (guint i = 0; i < priv->n_columns; i++)
...
ScpTreeStore *scp_tree_store_new(gboolean sublevels, gint n_columns, ...)
{
	...
	GType *types = g_malloc(n_columns * sizeof(GType));
	va_list ap;
	...
	ScpTreeStore *store = scp_tree_store_newv(sublevels, n_columns, types);
	g_free(types);
	return store;
}
...
ScpTreeStore *scp_tree_store_newv(gboolean sublevels, gint n_columns, GType *types)
{
	...
	ScpTreeStore *store = g_object_new(SCP_TYPE_TREE_STORE, "sublevels",
									   sublevels != FALSE, NULL);
...
#define scp_tree_model_compare_func \
	((GtkTreeIterCompareFunc)scp_tree_store_compare_func)

gboolean scp_tree_store_set_column_types(ScpTreeStore *store,
										 gint n_columns, GType *types)
{
	...
	priv->headers = scp_tree_data_headers_new(n_columns, types,
											  scp_tree_model_compare_func);
...
{
	if (store->priv->sort_order == GTK_SORT_ASCENDING)
		return result;
	else if (result > 0)
		return -1;
	else
...
static guint scp_search_pos(ScpTreeStore *store, GtkTreeIterCompareFunc func,
							gpointer data, GtkTreeIter *iter, gint low, gint high)
{
	if (low <= high)
	{
		GtkTreeIter iter1;
		...
		while (low < high)
		{
			...
			gint result = scp_store_compare(store, func, iter, &iter1, data);
...
static void scp_sort_element(ScpTreeStore *store, GtkTreeIter *iter,
							 gboolean emit_reordered)
{
	...
	if (index > 0)
	{
		...
		{
			scp_move_element(store, array, iter,
							 scp_search_pos(store, priv->sort_func, data, iter,
											0, index - 2),
							 emit_reordered);
			return;
		}
	...
	if (index < (gint)array->len - 1)
	{
		...
		if (scp_store_compare(store, priv->sort_func, iter, &iter1, data) > 0)
		{
			scp_move_element(store, array, iter,
							 scp_search_pos(store, priv->sort_func, data, iter,
											index + 2, array->len - 1) - 1,
							 emit_reordered);
		}
	}
}

static gboolean scp_set_value(ScpTreeStore *store, AElem *elem,
...
static void scp_set_vector(ScpTreeStore *store, AElem *elem, gboolean *changed,
						   gboolean *sort_changed, gint *columns,
						   ...
						   gboolean *sort_changed, va_list ap)
{
	...
	while ((column = va_arg(ap, int)) != -1)
	{
		...
		scp_tree_data_from_stack(elem->data + column, priv->headers[column].type,
								 ap, TRUE);
		*changed = TRUE;
...
static void scp_set_values_signals(ScpTreeStore *store, GtkTreeIter *iter,
								   gboolean changed, gboolean sort_changed)
{
	if (sort_changed)
...
void scp_tree_store_set_valuesv(ScpTreeStore *store, GtkTreeIter *iter,
...
void scp_tree_store_set_value(ScpTreeStore *store, GtkTreeIter *iter,
							  gint column, GValue *value)
{
...
gboolean scp_tree_store_remove(ScpTreeStore *store, GtkTreeIter *iter)
{
	...
	GPtrArray *array = ITER_ARRAY(iter);
	guint index = ITER_INDEX(iter);
	AElem *elem = (AElem *)array->pdata[index];
	AElem *parent = elem->parent;
								   ...
								   gint position, GtkTreeIter *parent_iter)
{
	...
	else
	{
		...
		position = 0;
		parent->children = array = g_ptr_array_sized_new(
										parent_iter ? priv->sublevel_reserved
...
void scp_tree_store_insert(ScpTreeStore *store, GtkTreeIter *iter,
...
void scp_tree_store_get_valist(ScpTreeStore *store, GtkTreeIter *iter, va_list ap)
{
	...
	while ((column = va_arg(ap, int)) != -1)
	{
		...
		gpointer dest = va_arg(ap, gpointer);
...
gboolean scp_tree_store_is_ancestor(VALIDATE_ONLY ScpTreeStore *store,
...
gint scp_tree_store_iter_depth(VALIDATE_ONLY ScpTreeStore *store, GtkTreeIter *iter)
{
	...
	gint depth = 0;
	for (AElem *elem = ITER_ELEM(iter); elem->parent; depth++, elem = elem->parent);
	
	return depth;
}

static void scp_clear_array(ScpTreeStore *store, GPtrArray *array,
...
static void scp_reorder_array(ScpTreeStore *store, GtkTreeIter *parent,
							  GPtrArray *array, gint *new_order)
{
	...
	/* emit signal */
	GtkTreePath *path = parent ? scp_tree_store_get_path(store, parent)
							   : gtk_tree_path_new();
...
void scp_tree_store_reorder(ScpTreeStore *store, GtkTreeIter *parent, gint *new_order)
{
	...
	if (array) scp_reorder_array(store, parent, array, new_order);
}
...
{
	for (guint i = 0; i < array->len; i++)
		if (array->pdata[i] == elem) return i;
...
gboolean scp_tree_store_get_iter(ScpTreeStore *store, GtkTreeIter *iter,
								 GtkTreePath *path)
{
	ScpTreeStorePrivate *priv = store->priv;
...
GtkTreePath *scp_tree_store_get_path(VALIDATE_ONLY ScpTreeStore *store,
									 GtkTreeIter *iter)
{
...
void scp_tree_store_get_value(ScpTreeStore *store, GtkTreeIter *iter,
							  gint column, GValue *value)
{
	ScpTreeStorePrivate *priv = store->priv;
	...
	scp_tree_data_to_value(ITER_ELEM(iter)->data + column,
						   priv->headers[column].type, value);
}

gboolean scp_tree_store_iter_next(VALIDATE_ONLY ScpTreeStore *store,
								  GtkTreeIter *iter)
{
	g_return_val_if_fail(VALID_ITER(iter, store), FALSE);
...
gboolean scp_tree_store_iter_previous(VALIDATE_ONLY ScpTreeStore *store,
									  GtkTreeIter *iter)
{
	g_return_val_if_fail(VALID_ITER(iter, store), FALSE);
									  ...
									  GtkTreeIter *parent)
{
	...
	ScpTreeStorePrivate *priv = store->priv;
	GPtrArray *array = (parent ? ITER_ELEM(parent) : priv->root)->children;
...
gboolean scp_tree_store_iter_has_child(VALIDATE_ONLY ScpTreeStore *store,
									   GtkTreeIter *iter)
{
	g_return_val_if_fail(VALID_ITER(iter, store), FALSE);
...
gint scp_tree_store_iter_n_children(ScpTreeStore *store, GtkTreeIter *iter)
{
	...
	GPtrArray *array = (iter ? ITER_ELEM(iter) : store->priv->root)->children;
									...
									GtkTreeIter *child)
{
	...
	AElem *parent = ITER_ELEM(child)->parent;
							...
							GtkTreeModelForeachFunc func, gpointer gdata)
{
	if (array)
	{
		...
		for (guint i = 0; i < array->len; i++)
		{
			...
			if (func((GtkTreeModel *) store, path, &iter, gdata)
				|| scp_foreach(store, ((AElem *)array->pdata[i])->children,
							   path, func, gdata))
				return TRUE;
...
void scp_tree_store_foreach(ScpTreeStore *store, GtkTreeModelForeachFunc func,
							gpointer gdata)
{
	g_return_if_fail(SCP_IS_TREE_STORE(store));
							 ...
							 GtkTreeIter *dest_iter)
{
	...
	if (elem->children)
	{
		...
		for (guint i = 0; i < elem->children->len; i++)
		{
			GtkTreeIter copy;
										   ...
										   GtkSelectionData *selection_data)
{
	...
	if (src_path) gtk_tree_path_free(src_path);
										   ...
										   GtkSortType *order)
{
	...
	if (sort_column_id) *sort_column_id = priv->sort_column_id;
	
	if (order) *order = priv->sort_order;
...
void scp_tree_store_set_default_sort_func(ScpTreeStore *store,
										  GtkTreeIterCompareFunc func,
										  gpointer data, GDestroyNotify destroy)
{
	ScpTreeStorePrivate *priv = store->priv;
	
	scp_tree_data_set_header(priv->headers, GTK_TREE_SORTABLE_DEFAULT_SORT_COLUMN_ID,
							 func, data, destroy);
									 ...
									 gpointer user_data, G_GNUC_UNUSED GError **error)
{
	...
	for (guint i = 0; names[i]; i++)
	{
		if (!strcmp(names[i], "type"))
		{
			...
			{
				g_warning("%s: Unknown type %s specified in treemodel %s",
						  G_STRFUNC, values[i], data->name);
			}
		...
		else if (!strcmp(names[i], "utf8_collate"))
		{
			...
			{
				g_warning("%s: %s for %s in treemodel %s", G_STRFUNC,
						  error->message, names[i], data->name);
				g_error_free(error);
			}
			else
			{
								   ...
								   G_GNUC_UNUSED GError **error)
{
	...
	if (!strcmp(element_name, "columns"))
	{
		...
		for (guint i = 0; i < data->collates->len; i++)
								   ...
								   guint sublevel_reserved, gboolean sublevel_discard)
{
	g_object_set(G_OBJECT(store), "sublevel-discard", sublevel_discard,
				 "sublevel_reserved", sublevel_reserved,
				 toplevel_reserved ? "toplevel-reserved" : NULL, toplevel_reserved,
				 NULL);
}

void scp_tree_store_set_utf8_collate(ScpTreeStore *store, gint column,
									 gboolean collate)
{
	g_return_if_fail(SCP_IS_TREE_STORE(store));
...
gint scp_tree_store_compare_func(ScpTreeStore *store, GtkTreeIter *a,
...
gboolean scp_tree_store_iter_seek(VALIDATE_ONLY ScpTreeStore *store,
...
static gboolean scp_linear_search(GPtrArray *array, gint column, ScpTreeData *data,
								  GType type, GtkTreeIter *iter, gboolean sublevels)
{
	if (array)
	{
		for (guint i = 0; i < array->len; i++)
		{
			...
			if (sublevels && scp_linear_search(elem->children, column,
											   data, type, iter, TRUE))
				return TRUE;
		}
	}
...
static gboolean scp_binary_search(GPtrArray *array, gint column, ScpTreeData *data,
								  GType type, GtkTreeIter *iter, gboolean sublevels)
{
	if (array)
	{
...
gboolean scp_tree_store_search(ScpTreeStore *store, gboolean sublevels,
							   gboolean linear_order, GtkTreeIter *iter,
							   GtkTreeIter *parent, gint column, ...)
{
	g_return_val_if_fail(SCP_IS_TREE_STORE(store), FALSE);
	g_return_val_if_fail(VALID_ITER_OR_NULL(parent, store), FALSE);
	...
	gboolean found = !linear_order && column == priv->sort_column_id &&
						(priv->sort_func == scp_tree_model_compare_func)
...
static gboolean scp_traverse(ScpTreeStore *store, GPtrArray *array,
							 GtkTreeIter *iter, gboolean sublevels,
							 ScpTreeStoreTraverseFunc func, gpointer gdata)
{
	if (array)
	{
		guint i = 0;
...
gboolean scp_tree_store_traverse(ScpTreeStore *store, gboolean sublevels,
								 ...
								 ScpTreeStoreTraverseFunc func, gpointer gdata)
{
	...
	if (!scp_traverse(store, array, iter, sublevels, func, gdata))
	{
		iter->stamp = 0;
		return FALSE;
	}
...
{
	GObject *object = G_OBJECT_CLASS(scp_tree_store_parent_class)
...
static void scp_tree_store_get_property(GObject *object, guint prop_id,
...
static void scp_tree_store_set_property(GObject *object, guint prop_id,
...
GType scp_tree_store_get_type(void)
{
	if (g_once_init_enter(&scp_tree_store_type_id_volatile))
	{
		GType type = g_type_register_static_simple(G_TYPE_OBJECT,
												   g_intern_string("ScpTreeStore"),
												   sizeof(ScpTreeStoreClass),
												   (GClassInitFunc)scp_tree_store_class_init,
												   sizeof(ScpTreeStore), NULL, 0);
		GInterfaceInfo iface_info =
				{ (GInterfaceInitFunc)scp_tree_store_tree_model_init, NULL, NULL };
