...
static void collect_tags(G_GNUC_UNUSED gpointer key, gpointer value,
						 gpointer user_data)
{
	...
	if (value) g_ptr_array_add(array, value);
}
...
void xproject_close(gboolean cache)
{
	...
	if (!g_current_project) return;
	
	cache ? g_ptr_array_add(g_projects, g_current_project)
...
void xproject_open(const gchar *path)
{
	...
	for (guint i = 0; i < g_projects->len; i++)
	{
	...
	if (!p) p = geany_project_load(path);
	if (!p) return;
	
	ui_set_statusbar(TRUE, _("Project \"%s\" opened."), p->name);
	
	GPtrArray *to_reload = g_ptr_array_new();
...
void xproject_update_tag(const gchar *filename)
{
	...
	for (guint i = 0; i < g_projects->len; i++)
	{
		tm_obj = (TMSourceFile *)g_hash_table_lookup(
									((struct GeanyPrj *)(g_projects->pdata[i]))->tags,
									filename);
		if (tm_obj)
		{	/* force tag update */
			tm_workspace_remove_source_file(tm_obj);
...
gboolean xproject_remove_file(const gchar *path)
{
	...
	TMSourceFile *tm_obj = (TMSourceFile *)g_hash_table_lookup(g_current_project->tags,
															   path);
	if (tm_obj) tm_workspace_remove_source_file(tm_obj);
