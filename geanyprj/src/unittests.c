...
gboolean true(G_GNUC_UNUSED const gchar *arg)
{
	return TRUE;
}
...
START_TEST(test_get_file_list)
{
	...
	fail_unless(g_slist_length(files) == 4, "expected %d, get %d", 4,
...
START_TEST(test_get_relative_path)
{
	...
	fail_unless(strcmp(newpath, "src/a.c") == 0, "expected %s, get %s", "src/a.c",
				newpath);
	g_free(newpath);
	
	newpath = get_relative_path("/home/yura/src/geanyprj/.geanyprj",
								"/home/yura/src/geanyprj");
