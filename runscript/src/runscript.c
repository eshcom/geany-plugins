								  ...
								  gpointer user_data)
{
	...
	GKeyFile *config = load_config_from_file(rs_info->config_file, NULL);
...
#define SAVE_CONF_TEXT(name) G_STMT_START {									\
	rs_info->name = gtk_editable_get_chars(									\
						GTK_EDITABLE(g_object_get_data(G_OBJECT(dialog),	\
													   "entry_" #name)),	\
						0, -1);												\
	g_key_file_set_string(config, CONFIG_SECTION, #name, rs_info->name);	\
} G_STMT_END
...
#undef SAVE_CONF_TEXT
	...
	write_config_to_file(config, rs_info->config_file, MSGBOX);
									  ...
									  G_GNUC_UNUSED gpointer pdata)
{
	...
	rs_info->config_file = get_config_filepath(PLUGIN, NULL);
...
#define GET_CONF_TEXT(name, hotkey_text) G_STMT_START {					\
	rs_info->name = utils_get_setting_string(config, CONFIG_SECTION,	\
											 #name, NULL);				\
...
#define WIDGET_CONF_TEXT(name, label_text) G_STMT_START {							\
	entry = add_inputbox(vbox, label_text, rs_info->name, -1,						\
						 _("git-repo-pull, git-repo-push, git-repo-commit, ..."),	\
						 FALSE, FALSE);												\
