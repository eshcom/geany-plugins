...
static PluginInfo *ac_info = NULL;

typedef struct {
	/* used to place the caret after setfiletyped items on tab (similar to eclipse) */
	gint jump_on_tab;
							 ...
							 gpointer user_data)
{
	...
	g_object_set_data_full(G_OBJECT(sci), "setfiletype-userdata", data, g_free);
}

static PluginCallback plugin_setfiletype_callbacks[] =
{
...
#define SAVE_CONF_BOOL(name) G_STMT_START {                                    \
	...
	g_key_file_set_boolean(config, "setfiletype", #name, ac_info->name);         \
} G_STMT_END
...
#define GET_CONF_BOOL(name, def) ac_info->name = utils_get_setting_boolean(config, "setfiletype", #name, def)
...
#define WIDGET_CONF_BOOL(name, description, tooltip) G_STMT_START {            \
	...
	WIDGET_CONF_BOOL(abracket_htmlonly, _("\tOnly for HTML"),
		_("SetFileType angular brackets only in HTML documents"));
	WIDGET_CONF_BOOL(dquote, _("Double quotes \" \""),
		_("SetFileType double quotes \" -> \"|\""));
	WIDGET_CONF_BOOL(squote, _("Single quotes \' \'"),
		_("SetFileType single quotes ' -> '|'"));
	WIDGET_CONF_BOOL(backquote, _("Backquote ` `"),
		_("SetFileType backquote ` -> `|`"));
	...
	WIDGET_CONF_BOOL(backquote_bashonly, _("\tOnly for Shell-scripts (Bash)"),
		_("SetFileType backquote only in Shell-scripts like Bash"));
	...
	WIDGET_CONF_BOOL(make_indent_for_cbracket, _("Indent when enclosing"),
		...
		"will setfiletype selected lines and make new block with indent."
	...
	WIDGET_CONF_BOOL(improved_cbracket_indent, _("Improved auto-indentation"),
		...
		"Works without \"setfiletype { }\" checkbox."));
	...
	WIDGET_CONF_BOOL(jump_on_tab, _("Jump on Tab to enclosed char"),
		_("Jump behind setfiletyped items on Tab press."));
...
static void setfiletype_cleanup(void)
{
	guint i = 0;
	...
	foreach_document(i)
	{
		...
		data = g_object_steal_data(G_OBJECT(sci), "setfiletype-userdata");
		g_free(data);
	}
}
...
static void plugin_setfiletype_help(G_GNUC_UNUSED GeanyPlugin *plugin,
								  G_GNUC_UNUSED gpointer pdata)
{
	utils_open_browser("http://plugins.geany.org/setfiletype.html");
}
...
void geany_load_module(GeanyPlugin *plugin)
{
	...
	plugin->info->description = _("SetFileType braces and brackets with lot of features");
	...
	plugin->funcs->help = plugin_setfiletype_help;
	plugin->funcs->callbacks = plugin_setfiletype_callbacks;
