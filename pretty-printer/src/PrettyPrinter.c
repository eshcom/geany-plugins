...
int processXMLPrettyPrinting(const char *xml, int xml_length, char **output,
							 int *output_length, PrettyPrintingOptions *ppOptions)
{
	/* empty buffer, nothing to process */
	if (xml_length == 0)
		return PRETTY_PRINTING_EMPTY_XML;
	...
	if (xmlPrettyPrinted == NULL)
	{
		PP_ERROR("Allocation error (initialisation)");
	...
	char *reallocated = (char *)g_try_realloc(xmlPrettyPrinted, xmlPrettyPrintedIndex);
	if (reallocated == NULL)
	{
...
void putCharInBuffer(char charToAdd)
{
	...
	if (xmlPrettyPrintedIndex >= xmlPrettyPrintedLength)
	{
		if (charToAdd == '\0')
			...
			xmlPrettyPrintedLength += inputBufferLength;
		
		char *reallocated = (char *)g_try_realloc(xmlPrettyPrinted,
												  xmlPrettyPrintedLength);
		if (reallocated == NULL)
		{
			PP_ERROR("Allocation error (char was %c)", charToAdd);
			return;
		}
		xmlPrettyPrinted = reallocated;
	}
...
bool isInlineNodeAllowed(void)
{
	...
	if (firstChar == '<')
	{
		...
		while (loop)
		{
			...
			if (current == closingComment && oldChar == closingComment)
				loop = FALSE; /* end of comment/cdata */
		...
		++currentIndex;
		while (isWhite(inputBuffer[currentIndex]))
			++currentIndex;
	}
	else
	{
		/* this is a text node. Simply loop to the next '<' */
		while (inputBuffer[currentIndex] != '<')
			++currentIndex;
	}
...
void processNode(void)
{
	...
	/* store the name */
	char *nodeName = (char *)g_try_malloc(sizeof(char) * nodeNameLength + 1);
	if (nodeName == NULL)
	{
		PP_ERROR("Allocation error (node name length is %d)", nodeNameLength);
		return;
	}
	...
	if (nextChar == '/') /* the node is being closed immediatly */
	{
		...
		{
			if (options->emptyNodeStrippingSpace)
				putCharInBuffer(' ');
			putNextCharsInBuffer(2);
		}
	...
	/* if the code reaches this area, then the processElements has been called
	 * and we must close the opening tag */
	char closeChar = getNextChar();
	if (closeChar != '<')
	{
		printError("processNode : Invalid character '%c' "
				   "for closing tag (should be '<')", closeChar);
		result = PRETTY_PRINTING_INVALID_CHAR_ERROR;
		return;
	}
...
void processComment(void)
{
	...
	if (options->inlineComment)
		inlineAllowed = isInlineNodeAllowed();
	if (inlineAllowed && !options->oneLineComment)
		inlineAllowed = isOnSingleLine(4, '-', '-');
	if (inlineAllowed)
		resetBackwardIndentation(TRUE);
...
void processTextNode(void)
{
	...
	if (options->inlineText)
		inlineTextAllowed = isInlineNodeAllowed();
	if (inlineTextAllowed && !options->oneLineText)
		inlineTextAllowed = isOnSingleLine(0, '<', '/');
	if (inlineTextAllowed || !options->alignText)
	{
		resetBackwardIndentation(TRUE); /* remove previous indentation */
		if (!inlineTextAllowed)
			putNewLine();
	}
...
{
	bool inlineAllowed = FALSE;
	if (options->inlineCdata)
		inlineAllowed = isInlineNodeAllowed();
	if (inlineAllowed && !options->oneLineCdata)
		inlineAllowed = isOnSingleLine(9, ']', ']');
	if (inlineAllowed)
		resetBackwardIndentation(TRUE);
	...
	while (loop)
	{
		...
		if (oldChar == ']' && nextChar == ']' && nextChar2 == '>')
			loop = FALSE; /* end of cdata */
	...
	while (loop)
	{
		...
		else if (isQuote(nextChar) || nextChar == '=')
		{
			if (nextChar == '=')
			{
				...
				if (!isQuote(nextChar))
				{
					printError("processDoctype : the next char should be a quote (not '%c')",
							   nextChar);
					result = PRETTY_PRINTING_INVALID_CHAR_ERROR;
					return;
				}
